...
{
public:
    HashSetQP(std::function<int(T)> h1) : h1{h1}, currentSize{DEFAULT_SIZE}, itemQty{0}
    {
    ...
    bool insert(const T& item)
    {
        ...
        // if subscript element neithy empty nor available, linear probe
        for (size_t i{0}; i < PROBE_MAX; i++)
        {
            if(c[(index + i*i) % (currentSize)].status == EMPTY || c[(index + i*i) % (currentSize)].status == AVAILABLE)
            {
                c[(index + i*i) % (currentSize)] = k;
                ++itemQty;  
                return true;           
            }
        }
    ...
    bool erase(const T& item)
    {
        ...
        if(c[index].status == AVAILABLE)
        {
            for (size_t i{0}; i < PROBE_MAX; i++)
            {
                if(c[(index + i*i) % (currentSize)].status == OCCUPIED && c[(index + i*i) % (currentSize)].key == item)
                {
                    c[(index + i*i) % (currentSize)].status = AVAILABLE;
                    --itemQty;
                    return true;
                }
            }
    ...
    const T* find(const T& item)
    {
            ...
            c[index].status == AVAILABLE)
        {
            ...
            {
                if(c[(index + i*i) % (currentSize)].status == OCCUPIED && c[(index + i*i) % (currentSize)].key == item)
                    return &c[(index + i*i) % (currentSize)].key;
            }
        }      
